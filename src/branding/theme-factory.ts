import { svSE } from '@mui/material/locale'
import { createTheme, type Theme } from '@mui/material/styles'
import { svSE as svSE_DataGrid } from '@mui/x-data-grid/locales'
import { toMap } from 'lib/to-map'
import type { Option } from '../options/types'
import type { ThemeModel } from './types'

const DEFAULT_DARK_THEME = createTheme({
    palette: {
        mode: 'dark',
    },
})

const DEFAULT_THEME_MODEL: ThemeModel = {
    'advert.image.aspectRatio': '4:3',
    'darkmode.enabled': 'true',
    'component.button.radius': '12',
    'component.button.elevation': 'true',
    'component.appbar.variant': 'outlined',
    'component.appbar.border': '0',
    'component.paper.variant': 'outlined',
    'component.avatar.variant': 'circular',
    'component.cardheader.variant': 'body1',
    'component.textfield.variant': 'outlined',
    'component.alert.variant': 'standard',
    'typography.body1.fontsize': '0.875',
    'shape.radius': '14',
    'custom.image.logotype':
        'data:image/svg+xml;base64,',
    'typography.font.family': 'Roboto',
    'cssbaseline.styleoverrides.fontface': '',
    // Light theme palette
    'light.palette.primary': '#76232f',
    'light.palette.secondary': '#ac8889',
    'light.palette.error': '#d32f2f',
    'light.palette.warning': '#ed6c02',
    'light.palette.info': '#ad8b5d',
    'light.palette.success': '#2e7d32',
    'light.palette.background': '#fff',
    'light.palette.paper': '#fff',
    'light.palette.text.primary': 'rgba(0,0,0,0.87)',
    'light.palette.text.secondary': 'rgba(0,0,0,0.6)',
    'light.palette.text.disabled': 'rgba(0,0,0,0.38)',
    'light.component.avatar.bgcolor': '#f6f3eb',
    'light.component.avatar.color': '#000000',
    'light.component.appbar.color': 'default',
    // Dark theme palette
    'dark.palette.primary': DEFAULT_DARK_THEME.palette.primary.main,
    'dark.palette.secondary': DEFAULT_DARK_THEME.palette.secondary.main,
    'dark.palette.error': DEFAULT_DARK_THEME.palette.error.main,
    'dark.palette.warning': DEFAULT_DARK_THEME.palette.warning.main,
    'dark.palette.info': DEFAULT_DARK_THEME.palette.info.main,
    'dark.palette.success': DEFAULT_DARK_THEME.palette.success.main,
    'dark.palette.background': DEFAULT_DARK_THEME.palette.background.default,
    'dark.palette.paper': DEFAULT_DARK_THEME.palette.background.paper,
    'dark.palette.text.primary': DEFAULT_DARK_THEME.palette.text.primary,
    'dark.palette.text.secondary': DEFAULT_DARK_THEME.palette.text.secondary,
    'dark.palette.text.disabled': DEFAULT_DARK_THEME.palette.text.disabled,
    'dark.component.avatar.bgcolor': '#dddddd',
    'dark.component.avatar.color': '#ffffff',
    'dark.component.appbar.color': 'default',
}

export const getDefaultThemeModel = (): ThemeModel => ({
    ...DEFAULT_THEME_MODEL,
})

const ThemModelKeys = new Set<keyof ThemeModel>([
    'advert.image.aspectRatio',
    'darkmode.enabled',
    'component.button.radius',
    'component.button.elevation',
    'component.appbar.variant',
    'component.appbar.border',
    'component.paper.variant',
    'component.avatar.variant',
    'component.cardheader.variant',
    'component.textfield.variant',
    'component.alert.variant',
    'typography.body1.fontsize',
    'custom.image.logotype',
    'shape.radius',
    'typography.font.family',
    'cssbaseline.styleoverrides.fontface',
    'light.palette.primary',
    'light.palette.secondary',
    'light.palette.error',
    'light.palette.warning',
    'light.palette.info',
    'light.palette.success',
    'light.palette.background',
    'light.palette.paper',
    'light.palette.text.primary',
    'light.palette.text.secondary',
    'light.palette.text.disabled',
    'light.component.avatar.bgcolor',
    'light.component.avatar.color',
    'light.component.appbar.color',
    'dark.palette.primary',
    'dark.palette.secondary',
    'dark.palette.error',
    'dark.palette.warning',
    'dark.palette.info',
    'dark.palette.success',
    'dark.palette.background',
    'dark.palette.paper',
    'dark.palette.text.primary',
    'dark.palette.text.secondary',
    'dark.palette.text.disabled',
    'dark.component.avatar.bgcolor',
    'dark.component.avatar.color',
    'dark.component.appbar.color',
])

export const createThemeModel = (options: Option[]): ThemeModel => ({
    ...getDefaultThemeModel(),
    ...toMap(
        options.filter(({ key }) => ThemModelKeys.has(key as keyof ThemeModel)),
        ({ key }) => key,
        ({ value }) => value
    ),
})

export const createThemeOptions = (model: ThemeModel): Option[] =>
    Object.entries(model).map(([key, value]) => ({
        key,
        value: String(value),
    }))

export const createCustomTheme = (
    model: ThemeModel,
    mode: 'light' | 'dark' = 'light'
): Theme => {
    const options = {
        ...getDefaultThemeModel(),
        ...model,
    }

    const fontSize = (value: string) => `${value}rem`

    return createTheme(
        {
            logotype: options['custom.image.logotype'],
            typography: {
                body1: {
                    fontSize: fontSize(options['typography.body1.fontsize']),
                },
                fontFamily: options['typography.font.family'],
            },
            palette: {
                mode,

                background: {
                    default: options[`${mode}.palette.background`],
                    paper: options[`${mode}.palette.paper`],
                },
                text: {
                    primary: options[`${mode}.palette.text.primary`],
                    secondary: options[`${mode}.palette.text.secondary`],
                    disabled: options[`${mode}.palette.text.disabled`],
                },
                primary: {
                    main: options[`${mode}.palette.primary`],
                },
                secondary: {
                    main: options[`${mode}.palette.secondary`],
                },
                error: {
                    main: options[`${mode}.palette.error`],
                },
                warning: {
                    main: options[`${mode}.palette.warning`],
                },
                info: {
                    main: options[`${mode}.palette.info`],
                },
                success: {
                    main: options[`${mode}.palette.success`],
                },
            },
            shape: {
                borderRadius: Number(options['shape.radius']),
            },
            components: {
                MuiButton: {
                    defaultProps: {
                        disableElevation:
                            options['component.button.elevation'] === 'true',
                    },
                    styleOverrides: {
                        root: {
                            borderRadius: Number(
                                options['component.button.radius']
                            ),
                        },
                    },
                },
                MuiCardHeader: {
                    defaultProps: {
                        slotProps: {
                            title: {
                                variant:
                                    options['component.cardheader.variant'],
                                sx: { fontWeight: 'bold' },
                            },
                        },
                    },
                },
                MuiLink: {
                    defaultProps: {
                        underline: 'hover',
                        color: 'text.primary',
                    },
                },
                MuiPaper: {
                    defaultProps: {
                        variant: options['component.paper.variant'],
                    },
                },
                MuiAppBar: {
                    defaultProps: {
                        enableColorOnDark: true,
                        elevation:
                            options['component.appbar.variant'] === 'outlined'
                                ? 0
                                : 4,
                        variant: options['component.appbar.variant'],
                        color: options[`${mode}.component.appbar.color`],
                    },
                    styleOverrides: {
                        root: {
                            border: options['component.appbar.border'],
                            ...(options[`${mode}.component.appbar.color`] ===
                                'default' && {
                                backgroundColor:
                                    options[`${mode}.palette.paper`],
                            }),
                        },
                    },
                },
                MuiAvatar: {
                    defaultProps: {
                        variant: options['component.avatar.variant'],
                    },
                    styleOverrides: {
                        root: {
                            color: options[`${mode}.component.avatar.color`],
                            backgroundColor:
                                options[`${mode}.component.avatar.bgcolor`],
                        },
                    },
                },
                MuiTextField: {
                    defaultProps: {
                        variant: options['component.textfield.variant'],
                    },
                },
                MuiSelect: {
                    defaultProps: {
                        variant: options['component.textfield.variant'],
                    },
                },
                MuiAlert: {
                    defaultProps: {
                        variant: options['component.alert.variant'],
                    },
                },
                MuiCssBaseline: {
                    styleOverrides:
                        options['cssbaseline.styleoverrides.fontface'] !== ''
                            ? `
                      @font-face {
                        font-family: ${options['typography.font.family']};
                        font-style: normal;
                        font-display: swap;
                        font-weight: 400;
                        src: url(${options['cssbaseline.styleoverrides.fontface']}) format('woff2');
                        unicodeRange: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF;
                      }
                    `
                            : undefined,
                },
            },
        },
        svSE,
        svSE_DataGrid
    )
}
