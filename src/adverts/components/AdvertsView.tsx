...
export const AdvertsView: FC = () => {
	const [ searchParams, setSearchParams ] = useLocalStorage<AdvertsSearchParams>(
		'haffa-live-search-v1',
		{
			search: '',
		})
	...
	return view({
		...
		resolved: (adverts, enqueue) => (
			<SearchableAdvertsList
				...
				{(adverts?.length === 0) && (
					<Box key='e'>
						<Phrase id='SEARCH_EMPTY_RESULT' value='Hoppsan, det blev inga träffar på den'/>
					</Box>)}
